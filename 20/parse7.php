<?php
  ...
  apply_picture_transform($tt);

  pretty_print();

  print "searching for seamonsters..\n";
  print "\n";
  ...
  $water_roughness = measure_water_roughness();

  print "water roughness is $water_roughness\n";
  ...
  function apply_picture_transform($transform) {
    global $picture;
    global $picture_w, $picture_h;
    if($transform["flip"] == 1) {
      $picture = apply_picture_flip();
    }
    for($i=0;$i<$transform["dir"];$i++) {
      $picture = apply_picture_90_rotation();
    }
  }

  function apply_picture_flip() {
    global $picture;
    global $picture_w, $picture_h;
    $new_data = array();
    for($y=0;$y < $picture_h; $y++) {
      $new_data[$y] = strrev($picture[$y]);
    }
    return $new_data;
  }

  function apply_picture_90_rotation() {
    global $picture;
    global $picture_w, $picture_h;
    $new_data = array();
    for($x=($picture_w-1);$x>=0;$x--) {
      $ss = "";
      for($y=0;$y<$picture_h;$y++) {
        $ss .= $picture[$y][$x];
      }
  ...
  function measure_water_roughness() {
    global $picture;
    global $picture_w, $picture_h;
    $roughness = 0;
    for ($y = 0; $y < $picture_h; $y++) {
  ...
  function find_seamonster() {
    global $picture_w, $picture_h;
    global $seamonster_w, $seamonster_h;
    $bound_w = ($picture_w - $seamonster_w + 1);
    $bound_h = ($picture_h - $seamonster_h + 1);
  ...
  function mark_seamonster($sx, $sy) {
    global $picture;
    global $picture_w, $picture_h;
    global $seamonster;
    global $seamonster_w, $seamonster_h;
  ...
  function find_seamonster_at($sx, $sy) {
    global $picture;
    global $picture_w, $picture_h;
    global $seamonster;
    global $seamonster_w, $seamonster_h;
  ...
  function pretty_print() {
    global $picture, $picture_w, $picture_h;;
      print "++++++++++++++++++\n";
    for($i = 0; $i<$picture_h; $i++) {
      //$check_w = strlen($picture[$i]);
      //$check_h = $i;
      //print "$check_w, $check_h   ";
      print "+".$picture[$i]."+\n";
    }
      print "++++++++++++++++++\n";
  }
